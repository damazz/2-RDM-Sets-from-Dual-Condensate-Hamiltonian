...
@numba.jit(nopython=True, nogil=True, parallel=True, cache = True)
def lamb(states, N, ind):
    st = states[:, :N] - states[:, N:]
    degen = np.zeros(st.shape[0], dtype = np.int64)
    for i in range(degen.shape[0]):
        ncnt, pcnt = cntter(st[i])
...
def Hinit(N, ind=False):
    if np.all(ind == False):
        ind = prune(N)
    size = comb1(2 * N, N)
    #    one = np.zeros(size)
    #    one[ind] = N
    #    epm = sparse.diagonalize(one)
    #    print("epm done")
    states = prep(N)
    states = states[ind]
    print("states done")
    lamm = sparse.COO(lamb(states, N, ind), data=1.0, shape=(size, size))
    print("lamm done")
    gamm = 2 * sparse.COO(gamma(states, N, ind), data=1.0, shape=(size, size))
    print("gamm done")
    gm = sparse.COO(G(states, N, ind), data=1.0, shape=gamm.shape)
    return lamm, gamm, gm, states
